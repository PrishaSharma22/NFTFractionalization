...
const STATUS_CONFIG: Record<string, { color: string; bg: string; border: string; dot: string; variant: "success" | "warning" | "info" }> = {
  Fractionalized: { color: "text-[#fbbf24]", bg: "bg-[#fbbf24]/10", border: "border-[#fbbf24]/20", dot: "bg-[#fbbf24]", variant: "warning" },
  Trading: { color: "text-[#4fc3f7]", bg: "bg-[#4fc3f7]/10", border: "border-[#4fc3f7]/20", dot: "bg-[#4fc3f7]", variant: "info" },
  Redeemed: { color: "text-[#34d399]", bg: "bg-[#34d399]/10", border: "border-[#34d399]/20", dot: "bg-[#34d399]", variant: "success" },
};

// ── Main Component ───────────────────────────────────────────

type Tab = "view" | "create" | "mint" | "buy";
...
export default function ContractUI({ walletAddress, onConnect, isConnecting }: ContractUIProps) {
  ...
  const [nftData, setNftData] = useState<Record<string, unknown> | null>(null);

  const truncate = (addr: string) => `${addr.slice(0, 6)}...${addr.slice(-4)}`;

  const handleCreateNft = useCallback(async () => {
    if (!walletAddress) return setError("Connect wallet first");
    if (!nftId.trim() || !nftName.trim() || !nftSymbol.trim() || !totalSupply.trim()) {
    ...
    try {
      ...
      setTxStatus("NFT fractionalized on-chain!");
      setNftId("");
      setNftName("");
      setNftSymbol("");
      setTotalSupply("");
  ...
  }, [walletAddress, nftId, nftName, nftSymbol, totalSupply]);

  const handleMintFraction = useCallback(async () => {
    if (!walletAddress) return setError("Connect wallet first");
    if (!mintNftId.trim() || !mintTo.trim() || !mintAmount.trim()) {
      return setError("Fill in all fields");
    }
    setError(null);
    setIsMinting(true);
    ...
    try {
      ...
      setTxStatus("Fractions minted on-chain!");
      setMintNftId("");
      setMintTo("");
      setMintAmount("");
      setTimeout(() => setTxStatus(null), 5000);
  ...
  }, [walletAddress, mintNftId, mintTo, mintAmount]);

  const handleBuyFraction = useCallback(async () => {
    if (!walletAddress) return setError("Connect wallet first");
    if (!buyNftId.trim() || !buyAmount.trim()) return setError("Fill in all fields");
    setError(null);
    setIsBuying(true);
    setTxStatus("Awaiting signature...");
    try {
      await buyFraction(walletAddress, buyNftId.trim(), parseInt(buyAmount, 10));
      setTxStatus("Fractions purchased on-chain!");
      setBuyNftId("");
      setBuyAmount("");
      setTimeout(() => setTxStatus(null), 5000);
    } catch (err: unknown) {
  ...
  }, [walletAddress, buyNftId, buyAmount]);
  ...
  const handleViewNft = useCallback(async () => {
    ...
    setNftData(null);
    try {
      const result = await getNft(viewNftId.trim(), walletAddress || undefined);
      if (result && typeof result === "object") {
        setNftData(result as Record<string, unknown>);
      } else {
  ...
  const tabs: { key: Tab; label: string; icon: React.ReactNode; color: string }[] = [
    ...
    { key: "create", label: "Create", icon: <PackageIcon />, color: "#7c6cf0" },
    { key: "mint", label: "Mint", icon: <NftIcon />, color: "#fbbf24" },
    { key: "buy", label: "Buy", icon: <BuyIcon />, color: "#34d399" },
  ];
        ...
        <AnimatedCard className="p-0" containerClassName="rounded-2xl">
          ...
          <div className="flex items-center justify-between border-b border-white/[0.06] px-6 py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-semibold text-white/90">FracNFT</h3>
          ...
          <div className="border-t border-white/[0.04] px-6 py-3 flex items-center justify-between">
            <p className="text-[10px] text-white/15">FracNFT &middot; NFT Fractionalization</p>

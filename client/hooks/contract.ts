...
export const CONTRACT_ADDRESS =
  "CDYXKDFWQC53JZFMDJVPC3G4PK7EFNUPBVISGGV255NN6Z6LRCBNIYQC";
...
// ============================================================
// NFT Fractionalization — Contract Methods
// ============================================================
...
/**
 * Initialize the fractionalization contract.
...
/**
 * Create a fractionalized NFT.
 * Calls: create_fractional_nft(nft_id: String, name: String, symbol: String, total_supply: u64)
 */
export async function createFractionalNft(
  caller: string,
  nftId: string,
  name: string,
  symbol: string,
  ...
  return callContract(
    ...
    [
      toScValString(nftId),
      toScValString(name),
      toScValString(symbol),
      toScValU64(totalSupply),
    ],
    caller,
    true
  );
...
/**
 * Mint fractions for a fractionalized NFT.
 * Calls: mint_fraction(nft_id: String, to: Address, amount: u64)
 */
...
/**
 * Buy fractions of a fractionalized NFT.
 * Calls: buy_fraction(nft_id: String, amount: u64)
 */
export async function buyFraction(
  ...
  return callContract(
    "buy_fraction",
    [toScValString(nftId), toScValU64(amount)],
    caller,
    true
  );
}
...
/**
 * Get fraction price per unit.
 * Calls: get_fraction_price(nft_id: String) -> i128
 */
export async function getFractionPrice(nftId: string, caller?: string) {
  return readContract(
    "get_fraction_price",
    [toScValString(nftId)],
    caller
...
/**
 * Get total fraction supply for an NFT.
 * Calls: get_fraction_supply(nft_id: String) -> u64
 */
export async function getFractionSupply(nftId: string, caller?: string) {
  return readContract(
    "get_fraction_supply",
    [toScValString(nftId)],
    caller
 ...
 * Calls: get_nft(nft_id: String) -> Map
 */
export async function getNft(nftId: string, caller?: string) {
  return readContract(
    "get_nft",
    [toScValString(nftId)],
    caller
